...
const buffer = fs_1.default.readFileSync("../../extension-poc.crx");
class LinkedinActions {
    ...
    static fetchLeads(searchTerm, userId) {
        ...
        return __awaiter(this, void 0, void 0, function*() {
            ...
            const searchBar = yield session.findElements(
                selenium_webdriver_1.By.className("search-global-typeahead__input")
            );
            yield searchBar[0].clear();
            yield searchBar[0].sendKeys(searchTerm);
            yield searchBar[0].sendKeys(selenium_webdriver_1.Key.ENTER);
            yield session.wait(
                selenium_webdriver_1.until.elementLocated(
                    selenium_webdriver_1.By.xpath(`//button[contains(text(), People)]`)
                ),
                10000
            );
            const filterButtons = yield session.findElements(
                selenium_webdriver_1.By.className(
                    "search-reusables__filter-pill-button"
                )
            );
            let peopleButton = yield Promise.all(
            ...
            yield (_a = peopleButton[0]) === null || _a === void 0
                ? void 0
                : _a.click();
            yield session.wait(
                selenium_webdriver_1.until.elementLocated(
                    selenium_webdriver_1.By.className("entity-result")
                ),
                10000
            );
            const extensionButton = yield session.findElements(
                selenium_webdriver_1.By.id("realply_icon")
            );
            yield extensionButton[0].click();
            const startCampainButton = yield session.findElements(
                selenium_webdriver_1.By.id("start_campaign")
            );
    ...
    static sendReuqest(requestData) {
        return __awaiter(this, void 0, void 0, function*() {
            ...
            yield extensionButton[0].click();
            const sendReuqest = yield session.findElements(
                selenium_webdriver_1.By.id("send_request")
            );
